    ...
    impure subroutine s_check_inputs
        ...
        if (amr) then
            ...
            @:PROHIBIT(amr_ref_ratio /= 2 .and. amr_regrid_int /= 0, &
                       & "amr_ref_ratio /= 2 with dynamic regrid (amr_regrid_int > 0) is not supported: dynamic regrid tracks a " &
                       & // "MOVING interface, and a moving sharp surface-tension interface is numerically unstable at " &
                       & // "ref_ratio > 2 (CSF parasitic currents amplified by the finer interface - a solver limitation, not " &
                       & // "an AMR bug; the regrid migration itself is conservative). Use amr_regrid_int = 0 (static block)")
