...
ax.imshow(grad, extent=(0, L, 0, band), origin="lower", aspect="auto",
...
ax.add_patch(Rectangle((0, -2.6), L, 2.6, facecolor="0.8", edgecolor="k",
                       hatch="////", lw=1.5, zorder=4))
ax.plot([0, L], [0, 0], "k-", lw=3.0, zorder=5)
ax.text(L / 2, -6.6, "Flat plate — no-slip isothermal wall,  $T_{wall}=600$ K   (bc_y%beg = −16)",
        ha="center", va="center", fontsize=10.5, fontweight="bold")

# -- edge boundary-condition arrows --
# left edge: subsonic inflow (arrows pointing in, +x)
for f in np.linspace(0.18, 0.82, 4):
    ax.add_patch(FancyArrowPatch((-6.5, L * f), (-1.0, L * f),
                 arrowstyle="-|>", mutation_scale=13, color="C0", lw=1.6, zorder=6))
ax.text(-9.5, L / 2, "Subsonic inflow\n(bc_x%beg = −7)", rotation=90, ha="center",
        va="center", fontsize=10, color="C0")

# right edge: outflow (arrows pointing out, +x)
for f in np.linspace(0.18, 0.82, 4):
    ax.add_patch(FancyArrowPatch((L + 1.0, L * f), (L + 6.5, L * f),
                 arrowstyle="-|>", mutation_scale=13, color="0.35", lw=1.6, zorder=6))
ax.text(L + 9.5, L / 2, "Outflow\n(bc_x%end = −3)", rotation=90, ha="center",
        va="center", fontsize=10, color="0.3")

# top edge: outflow (arrows pointing out, +y)
for f in np.linspace(0.15, 0.85, 5):
    ax.add_patch(FancyArrowPatch((L * f, L + 1.0), (L * f, L + 6.5),
                 arrowstyle="-|>", mutation_scale=13, color="0.35", lw=1.6, zorder=6))
ax.text(L / 2, L + 9.0, "Outflow  (bc_y%end = −3)", ha="center", va="center",
...
ax.annotate("Thermal boundary layer\n$\\delta_T=\\sqrt{\\alpha t}\\approx 3$ mm at 5 ms\n(uniform in $x$, grows in time)",
            xy=(L * 0.80, band * 0.5), xytext=(L * 0.62, band + 9.5),
...
for label, val in lines:
    header = (val == "")
    axp.text(0.0, y, label, fontsize=10.0 if header else 9.3,
             fontweight="bold" if header else "normal",
             color="#7a3b00" if header else "k", transform=axp.transAxes)
    if val:
