...
def main():
    ...
    steps = sorted(
        ...
        if (g_ := re.search(r"lustre_(\d+)\.dat$", os.path.basename(f)))
    )
    ...
    rho_inf = float(np.fromfile(os.path.join(rd, "lustre_0.dat"), np.float64)
                    .reshape(-1, ncy, ncx)[0].mean())
    ...
    ax[0].plot(theta_an(yc, t_last, alpha_film), yc * 1e3, "k--", lw=2,
               label=f"erf, α_film={alpha_film:.2e}")
    ax[0].set(xlabel="θ = (T−T_wall)/(T_inf−T_wall)", ylabel="y [mm]",
              title="T profiles (color=time) vs erf", ylim=(0, 6))
    ax[0].legend()
    ax[0].grid(alpha=0.3)
    ...
    ax[1].set(xlabel="η = y / (2√(α_film t))", ylabel="θ",
              title="self-similar collapse", xlim=(0, 3))
    ax[1].legend()
    ax[1].grid(alpha=0.3)
    fig2.tight_layout()
    ...
    print(f"  mean rms error vs erf(alpha_film) = {np.mean(rms_all):.2f} K "
          f"({100*np.mean(rms_all)/(T_inf-T_wall):.1f}% of ΔT)")
    print(f"  wrote validation_field.png, validation_profiles.png")
