...
10 = color. All read from each run's simulation.inp so it can't silently slice wrong.
...
def main():
    ...
    print(f"{os.path.basename(run)} step={step} t/t_r={step*dt/t_r:.3f}: " f"max|vy|={np.abs(vy).max():.4e}  max|v|={vmag.max():.4e}  " f"max|vy| in far-field rows={np.abs(vy[far]).max():.4e}")
    print(f"  wrote {dst}")
