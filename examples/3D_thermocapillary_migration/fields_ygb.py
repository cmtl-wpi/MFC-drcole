...
Renders an x-y midplane (z = Lz/2) of the independent thermal scalar T_s with the drop's color
interface overlaid, plus the centerline T_s(y) profile against the imposed linear field and the
isothermal-wall temperatures. Confirms the IC is what case_ygb.py intended: a linear, wall-pinned
T_s and a coherent spherical color patch.
...
# thermal_scalar appends T_s last, color second-to-last.
Ts = field(nvars - 1)
color = np.clip(field(nvars - 2), 0.0, 1.0)
kz = nz // 2  # z midplane
Ts_mid, c_mid = Ts[kz], color[kz]  # (ny, nx)
...
mesh = ax_f.pcolormesh(x, y, Ts_mid, cmap="coolwarm", shading="auto")
fig.colorbar(mesh, ax=ax_f, label=r"thermal scalar $T_s$")
...
ax_f.set(aspect="equal", xlabel="x", ylabel="y (rise axis)", title=f"$T_s$ + color, z-midplane (step {step}, $t/t_r$={step * dt / t_r:.3f})\ndrop centroid y={yc:+.4f}")

mid_x = nx // 2
ax_l.plot(Ts_mid[:, mid_x], y, "-", color="C3", lw=1.8, label="MFC $T_s$ centerline")
...
ax_l.set(xlabel=r"$T_s$", ylabel="y (rise axis)", title="centerline $T_s(y)$ vs imposed gradient")
...
print(f"T_s range [{Ts.min():.4f}, {Ts.max():.4f}]  walls [{Tw_in:.4f}, {Tw_out:.4f}]  drop y={yc:+.4f}")
print(f"saved -> {out}")
