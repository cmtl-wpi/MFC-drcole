...
def main():
    with plt.rc_context(PLATE_STYLE):
        ...
        ax.annotate("Ma=0.03 at big dt:\nICFL blow-up, step 13", xy=(13 * 2.668e-3 / 7.5, 2.4), xytext=(0.25, 2.15), fontsize=8.5, color="k", arrowprops=dict(arrowstyle="->", color="0.4"))
        ax.set_xlim(0, 0.65)
        ax.set_ylim(-0.3, 2.6)
        ax.set_xlabel(r"$t / t_r$")
        ax.set_ylabel(r"rise velocity   $v / v_{\mathrm{YGB}}$")
        ax.set_title("3D thermocapillary @ Nx=32: dt sets stability, not Ma\n" "(at fixed Ma, big dt unstable / small dt stable -- both directions)", fontsize=11, loc="left")
