...
SILO = os.environ.get("MFC_SILO", "/home/daveygravy/repos/MFC/examples/3D_droplet_coalescence/silo_hdf5")
...
def main():
    ...
    smp = verts.T
    n = -np.stack([map_coordinates(gi, smp, order=1), map_coordinates(gj, smp, order=1), map_coordinates(gk, smp, order=1)], axis=1)
    ...
    print(f"ts={ts} field={sys.argv[3] if len(sys.argv)>3 else 'alpha1'} " f"grid={G.shape} verts={len(V)} faces={len(faces)} " f"bbox(units)={ext.round(3)} rmax={rmax:.3f}")
