% 1D Dirichlet sine decay, energy-coupled -- case_1d_energy.py.
% Same physical benchmark as case_1d, but T is the EOS temperature: the sine
% profile is imposed through the DENSITY at uniform pressure,
%   rho(x) = rho0 Twall / (Twall + A sin(pi x/L)),   p = p0 = const.
% T is then recovered from the stiffened-gas EOS; weak compressible acoustics
% make u != 0 and set an error floor (vs the clean scalar operator).
%
...
\node[font=\bfseries\small, anchor=south west] at (-0.9,4.7)
  {1D sine decay, energy-coupled \ \footnotesize(\texttt{case\_1d\_energy.py})};
\node[anchor=north west, align=left, font=\footnotesize] at (-0.9,-1.05)
  {$\rho(x)=\dfrac{\rho_0 T_{\text{wall}}}{T_{\text{wall}}+A\sin(\pi x/L)}$ imposes the same $T(x)$ as the scalar case;\;\;
