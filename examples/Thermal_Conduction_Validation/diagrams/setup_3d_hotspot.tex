...
%   T_s = T0 + A exp(-r^2 / 2 sigma0^2),  sigma0 = 0.08,
...
\begin{tikzpicture}[>=Stealth, font=\footnotesize, line cap=round]
  ...
  \def\dx{1.7}
  \def\dy{1.25}
  \coordinate (C) at (\S/2,\S/2);  % blob center on front face
  ...
  \node[inkc, anchor=west, align=left] at (\S+\dx+0.7,\S-0.3)
    {\textbf{3D Gaussian hot spot}\\[2pt]
     \texttt{case\_3d\_hotspot.py}\\[4pt]
     $T_s=T_0+A\,e^{-r^2/2\sigma_0^2}$\\
     $\sigma_0=0.08,\ A=5$\\[4pt]
     spreads as a heat kernel:\\
     $\sigma^2=\sigma_0^2+2\alpha t$\\
     peak $\sim(\sigma_0/\sigma)^3$\\[4pt]
     run until $\sigma$ doubles,\ $u\equiv 0$};
\end{tikzpicture}
\end{document}
