...
def main():
    ...
    with plt.rc_context(P.PLATE_STYLE):
        ...
        ax.plot(P.SAMAREH_VOF[:, 0], P.SAMAREH_VOF[:, 1], "s--", color="0.0", ms=5.0, mfc="none", mew=1.3, lw=1.0, label=r"Samareh Fig 5(d), VOF (digitized)")
        ax.axhline(1.0, color="0.3", lw=1.0, ls="--", zorder=1, label=r"$u_{\mathrm{YGB}}$ (analytic terminal)")
        n = 0
        for ma_txt, base, sharp, color in PAIRS:
            cb, cs = curve(base), curve(sharp)
            if cb is not None:
                ax.plot(cb[0], cb[1], "o--", color=color, ms=3.5, mew=0, lw=0.9, alpha=0.55, label=rf"{ma_txt}, $w=2\,\Delta x$ (smooth_coeff 0.5)")
                n += 1
            if cs is not None:
                ax.plot(cs[0], cs[1], "o-", color=color, ms=4.0, mew=0, lw=1.6, label=rf"{ma_txt}, $w\approx\Delta x$ (smooth_coeff 0.99)")
                n += 1
        ax.set_xlim(0.0, 10.0)
        ax.set_ylim(0.0, 1.1)
        ax.set_xlabel(r"Time   $t/t_r$")
        ax.set_ylabel(r"Normalized Rise Velocity   $u/u_{\mathrm{YGB}}$")
        ax.set_title(r"Fig 5 — interface-thickness effect on rise velocity ($Ma\to0$)", fontsize=12)
