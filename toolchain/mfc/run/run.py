...
def run(targets=None, case=None):
    ...
    # Pre-flight --archive: validate the path/format and reserve a unique
    # run directory BEFORE generating inputs, then redirect the run into it
    # so its inputs and outputs are written there directly (instead of next
    # to case.py). Failing here saves the user a full simulation if the
    # archive settings are bad. Done after build() because the build slug is
    # independent of the case directory. Batch jobs skip archiving entirely
    # (outputs land asynchronously).
    archive_plan = None
    if ARG("archive") is not None:
        if isinstance(qsystem, queues.InteractiveSystem):
            archive_plan = archive_mod.plan_archive(case)
            archive_mod.prepare_run_dir(archive_plan, case)
            cons.print(f"Running in [magenta]{archive_plan.run_dir}[/magenta].")
        else:
    ...
    if not ARG("dry_run"):
        ...
        if archive_plan is not None:
            archive_mod.finalize_archive(archive_plan, case, targets)
