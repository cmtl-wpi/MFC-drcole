...
_CMAP_POPULAR = "viridis, plasma, inferno, magma, turbo, coolwarm, RdBu_r, bwr, hot, jet, gray, seismic"
...
def _parse_step_list(s, available_steps):
    ...
    if "..." in parts:
        idx = parts.index("...")
        if idx < 2:
            raise MFCException(f"Invalid --step value '{s}'. Ellipsis '...' requires at least two values before it, e.g. '0,100,...,1000'.")
        if idx != len(parts) - 2:
            raise MFCException(f"Invalid --step value '{s}'. Ellipsis '...' must be the second-to-last item, e.g. '0,100,...,1000'.")
        try:
            prefix = [int(p) for p in parts[:idx]]
            end = int(parts[idx + 1])
        except ValueError as exc:
            raise MFCException(f"Invalid --step value '{s}': all values must be integers.") from exc

        stride = prefix[-1] - prefix[-2]
        if stride <= 0:
            raise MFCException(f"Invalid --step value '{s}': ellipsis stride must be positive (got {stride}).")
...
def _parse_steps(step_arg, available_steps):
    ...
    try:
        if ":" in s:
            parts = s.split(":")
            if len(parts) > 3:
                raise MFCException(f"Invalid range '{step_arg}': expected start:end or start:end:stride.")
            start = int(parts[0])
            end = int(parts[1])
...
def viz():
    ...
    if fmt_arg:
        if fmt_arg not in ("binary", "silo"):
            raise MFCException(f"Unknown format '{fmt_arg}'. Supported formats: 'binary', 'silo'.")
        fmt = fmt_arg
    else:
        try:
            ...
            if os.path.isfile(os.path.join(case_dir, "case.py")):
                msg += " This looks like an MFC case directory. Did you forget to run post_process first?"
    ...
    if ARG("list_vars"):
        ...
        steps = discover_timesteps(case_dir, fmt)
        if not steps:
            raise MFCException(f"No timesteps found in '{case_dir}' ({fmt} format). Ensure post_process has been run and produced output files.")
        ...
        if step_arg is None or step_arg == "all":
            ...
            try:
                ...
                raise MFCException(f"Invalid --step value '{step_arg}'. Expected an integer, 'last', or 'all'.") from exc
            if step not in steps:
                raise MFCException(f"Timestep {step} not found. Available steps: {_steps_hint(steps)}")
    ...
    if not requested_steps:
        ...
        raise MFCException(f"No matching timesteps for --step {step_arg!r}{detail}. Available steps: {_steps_hint(steps)}")
    ...
    if test_assembled.ndim == 3 and len(requested_steps) > _3d_limit:
        raise MFCException(f"Refusing to load {len(requested_steps)} timesteps for 3D data (limit is {_3d_limit}). Use --step with a range or stride to reduce.")
    ...
    if tiled and not interactive and not use_tui:
        if test_assembled.ndim == 3:
            ...
            tiled = False
            cons.print(f"[dim]Auto-selected variable: [bold]{varname}[/bold] (use --var to specify)[/dim]")
    ...
    if not tiled and not interactive and not use_tui and varname not in test_assembled.variables:
        ...
        raise MFCException(f"Variable '{varname}' not found. Available: {avail_str}. Use --list-vars to see variables at a given step.")

    # TUI mode — launch Textual terminal UI (1D/2D only)
    if use_tui:
        if test_assembled.ndim == 3:
            raise MFCException("Terminal UI only supports 1D and 2D data. Use --interactive for 3D data.")
        from .tui import run_tui

        init_var = varname if varname in avail else (avail[0] if avail else None)
        run_tui(init_var, requested_steps, read_step, ndim=test_assembled.ndim, bubble_func=bubble_func)
        return

    # Interactive mode — launch Dash web server
    if interactive:
        ignored = [f for f in ("slice_index", "slice_value") if ARG(f) is not None]
        if ARG("slice_axis") != "z":
            ignored.insert(0, "slice_axis")
        if ignored:
            cons.print(f"[yellow]Warning:[/yellow] {', '.join('--' + f.replace('_', '-') for f in ignored)} ignored in --interactive mode (use the UI controls instead).")
    ...
    if ARG("mp4"):
        ...
        if fps == _FPS_DEFAULT and n_frames / fps < _MIN_DURATION:
            ...
            cons.print(f"[dim]Auto-adjusted fps to {fps:.2g} so video is at least {_MIN_DURATION:.0f}s (use --fps to override).[/dim]")
        ...
        try:
            success = render_mp4(varname, requested_steps, mp4_path, fps=fps, read_func=read_step, tiled=tiled, bubble_func=bubble_func, **render_opts)
        except ValueError as exc:
            raise MFCException(str(exc)) from exc
        if success:
            cons.print(f"[bold green]Done:[/bold green] {mp4_path}")
        else:
            raise MFCException(f"Failed to generate {mp4_path}. Ensure imageio and imageio-ffmpeg are installed.")
        return
    ...
    for step in step_iter:
        ...
        if tiled and assembled.ndim == 1:
            render_1d_tiled(assembled.x_cc, assembled.variables, step, output_path, **step_opts)
    ...
    if failures:
        cons.print(f"[yellow]Warning:[/yellow] {len(failures)} step(s) failed: {failures[:10]}{'...' if len(failures) > 10 else ''}")
    if rendered > 1:
