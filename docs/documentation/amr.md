...
| Interface compression — directional THINC (`int_comp = 1`) | Supported | The 1D directional compression is buffer-free and reads the live (swapped) block arrays, so it sharpens the volume fraction on each level; recommended for multi-fluid interface problems under AMR |
| Interface compression — MTHINC (`int_comp = 2`) | **Not supported** | The normal/position buffers (`mthinc_nhat`, `mthinc_d`) are sized once to the base-grid `idwbuff` in `s_initialize_thinc_module`, but the fine advance swaps `idwbuff` to the block's larger bounds before the normals kernel overruns them; needs per-block buffers first (checker gate) |
